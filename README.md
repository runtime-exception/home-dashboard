...
- **强调色**可以直接用取色器挑，也可以点下面一排预设色块（取自门户里已经在用的配色，保证和既有卡片同一套视觉）。留空表示不设强调色。
- **图标地址**旁的「自动获取」会去抓该工具的公网地址（没填公网才回落到内网），从页面声明的 `<link rel="icon">` 或 web app manifest 里推断图标，并且逐个验证确实返回图片（声明了却 404 的会被跳过），最后才按常见路径兜底。解析不出来会提示手工填写。
